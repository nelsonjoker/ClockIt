...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public Camera.CameraInfo getCameraInfo() { return mCameraInfo; }
...
*/
        //instead adjust to the most similar width and height
        double bestDWidth = reqPreviewWidth;
        double bestDHeigth = reqPreviewHeight;
        Camera.Size retSize = null;
        for (Camera.Size size : mPreviewSizeList) {
            double dH = Math.abs(1 - reqPreviewHeight / (1.0*size.height));
            double dW = Math.abs(1 - reqPreviewWidth / (1.0*size.width));

            if(dH <= bestDHeigth && dW <= bestDWidth){
                bestDWidth = dW;
                bestDHeigth = dH;
                retSize = size;
            }
...
*/
            angle = getCorrectCameraOrientation(mCameraInfo, mCamera);
...
/*
        if(cameraParams.isZoomSupported()) {
            //int MAX_ZOOM = cameraParams.getMaxZoom();
...
*/
    ...
    protected int getCorrectCameraOrientation(Camera.CameraInfo info, Camera camera) {
    ...
    public boolean isPortrait() {
        //int rotation = mActivity.getWindowManager().getDefaultDisplay().getRotation();
        //return rotation == 1 || rotation == 270;
